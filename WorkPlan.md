...
**Plan version:** v1.1 (contract-audit release: dual-gate execution semantics via v0.4.3, per-track NOT_REACHED, theorem-file-once + math/proofs/, explicit Phase-2 PROOF CONSTRUCTION, corrected counts/wording/ownership; versioned by git history, never silently edited after `FOUNDATION_FROZEN`)
...
**Exit criteria.** PROVE track: entry gate satisfied (all five upstream REVIEWED) and REVIEWED + terminal, or REFUTED + counterexample; REFUTE track: runs on `REFUTE_READY` regardless. Exact REFUTED on either track freezes later POSITIVE-route phases (`NOT_REACHED`) and permits Phase-6 activation; `REP-01..14` green; threats `T043–T052` controlled.
...
**Exit criteria.** PROVE track: entry gate satisfied (MST0-14 REVIEWED) and REVIEWED + terminal, or REFUTED; REFUTE track: runs on `REFUTE_READY` regardless. Exact REFUTED on either track freezes later POSITIVE-route phases (`NOT_REACHED`) and permits Phase-6 activation; `INT-01..14` green; threats `T053–T063` controlled.

---

## 8. Phase 5 — Composition endgame: Pair Access + telescope + Levy–Tarjan bridge (spec PHASE 14–16)

**Scope.** Dual-gate execution (v0.4.3 C1–C3): `REFUTE(MST0-17)` runs on `REFUTE_READY` with NO upstream-REVIEWED requirement (its endpoint-aware negation instantiates from the frozen statement + frozen semantics alone — there is no mathematical dependence on unproven premises to instantiate a search); `PROVE(MST0-17/18/19)` runs only if all seven upstream nodes are REVIEWED, else the PROVE tracks alone are `NOT_REACHED`. Reconstruct Pair Access from scratch (no status-toggling), control endpoints to approximate monotonicity, audit the exact bridge (DEC-GATE-11..13 → `BRIDGE_READY_FOR_DECISION`).
...
**Files to be made.** `python/negative/{lift_obstruction,splay_lower,competitor_upper,family_replay}.py` + `lean/Proofs/Family.lean` (negative proof importing frozen declarations, independent of failed ledger) + `math/negative/` + reviews + `artifacts/v04/negative/` + `proofs/decision.json` + `tests/negative/` (`NEG-01..12`) + `scripts/run_phase17.py`, `run_phase18.py`.
...
- `PROVE(L)` may execute only whenever `PROVE_READY(L)` holds: Phase-3 PROVE needs all five upstream REVIEWED; Phase-4 PROVE needs MST0-14 REVIEWED; Phase-5 PROVE needs all seven upstream REVIEWED.
