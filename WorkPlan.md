...
- MST0-08U PROOF CONSTRUCTION: human proof in `math/proofs/MST0-08U_proof.md` establishing the universal constant bound on exactly the downstream-consumed primitive modifications (injection ≤k/rotation + activation ≤1/event + payment accounting), arbitrary n, no finite-maximum premise, no corpus/seed/tree-size dependence; Lean theorem in `Locality.lean` against the frozen declaration; proof certificate binding statement/negation/dependency hashes; PSC-L result attached as attack evidence only.
- MST0-11 PROOF CONSTRUCTION: human proof in `math/proofs/MST0-11_proof.md` covering ROOT/ZIG-left/ZIG-right/LL/RR/LR/RL × every T5/T6/T7 branch over arbitrary intervals/sizes/ranks/supports/provenance, discharging all 9 preservation obligations symbolically (never finite enumeration as premise); Lean theorem in `Preservation.lean`; certificate + attack evidence as above.
- MST0-09 PROOF CONSTRUCTION: human proof in `math/proofs/MST0-09_proof.md` identifying the exact cost-bearing source of every harmful boundary contribution (nested/alternating/creation-rate/rank-gap/span/burden/burst/lifetime/reactivation/asymmetry/mirror/scale cases) with the universal bound; Lean theorem in `Boundary.lean`; certificate + attack evidence as above.
- MST0-22 PROOF CONSTRUCTION: human proof in `math/proofs/MST0-22_proof.md` showing the frozen definitions themselves are uniform (C=2/k=6 as quantified-before-arbitrary-inputs literals with dataflow independence from all 9 forbidden parameters; scanner output attached as support, never as the proof); Lean theorem with formal quantifier-order check in `Constants.lean`; certificate + hidden-dependence attack result as above.
...
**Deterministic order (15 steps with bifurcated gate asserts per v0.4.3 C2).** Every phase script asserts all 15 with the gate split explicit: ASSERT FOUNDATION / REFUTATION GATES before LOAD THEOREM + NEGATION and RUN REFUTATION ATTACK, then ASSERT POSITIVE PROOF DEPENDENCY GATES before DEVELOP / CHECK PROOF; violations fail closed. Parallelism only for independent searches/cases/replays/mutations/modules/prefixes with deterministic post-sort.
...
WorkPlan phases execute 1→7 (spec PHASE order preserved inside each phase). Within every phase the §21 15-step order is mandatory with the v0.4.3 C2 bifurcation: `ASSERT FOUNDATION / REFUTATION GATES` precedes `LOAD THEOREM + NEGATION` and `RUN REFUTATION ATTACK`, while `ASSERT POSITIVE PROOF DEPENDENCY GATES` precedes `DEVELOP / CHECK PROOF` (a single blanket gate assert would prohibit the independent REFUTE track). `Path.md` is appended contemporaneously after every phase-step with: implementation, WorkPlan section followed (or justified deviation), scope/files/code/how/benchmarks/anti-overfitting/gates detail, verification evidence (tests/hashes/review IDs), and next blockers. A gate without a `Path.md` entry is not closed (`INV-097`).
