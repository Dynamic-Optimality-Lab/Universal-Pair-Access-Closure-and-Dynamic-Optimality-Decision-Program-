...
**Authority/mechanism.** v0.4.8 (H1 authorization + exact mapping; H2 SUPERSEDED mechanics + 45/44 materialization + byte-stability rule; H3 effect). Old freeze preserved: `SUPERSEDED_prereg_sha256_17bacf8c.txt` + `SUPERSEDED_PATH_AT_FOUNDATION_FREEZE_1e9fc63e.md` + `SUPERSEDED_PREREG_PLACEHOLDER_REPAIR.json`. Regenerated: `prereg_sha256.txt` (57 members), `FOUNDATION_FROZEN.json` (manifest `6dbb767a…`, supersession pointer), PATH snapshot (through R1-023). PROOF_STATUS snapshot untouched (bytes unchanged). Normative-stack declarations wired (WorkPlan v1.6→v1.7; v1.6 pinned by superseded manifest + history).
**Values.** Parent ten hashes verified = SHA-256 of the R1-021-verified `parent/` bytes (SEAL `7ec1650e…`, FINAL_RESULT `7ce226ca…`, MANIFEST `907e2a37…`, ARCHIVE `b5ab0848…`, PATH_FINAL `b5e9a5e6…`, WORKPLAN_FINAL `12edc94b…`, MSTC_0002 `0c2bba6a…`, THEOREM_STATUS `32551d33…`, COUNTEREXAMPLE_INDEX `433c27a9…`, BOOTSTRAP `ae21d702…`). L0 `hash:` = `NOT_APPLICABLE_BY_PARENT_BOUND` + authority pointer (no authority issues a single SHA-256 of the release; inventing one refused). Section key renamed `pending_Phase-1-hashes` → `verified_Phase-1-hashes` (no consumers).
**Transitive findings (substantive).** T-C6: committed manifest bytes (LF, `17bacf8c`) vs ceremony working bytes (CRLF, `83c8690f`) — `core.autocrlf` conversion made repo/working hashes differ, breaking fresh-checkout recomputation. Root fix: `.gitattributes * -text` (H2 byte-stability rule); manifest recomputed stable (`6dbb767a`, 57/57 recompute-verified). Old record keeps both hashes with the ending note. T-C7: R1-023-above-marker broke the snapshot prefix invariant (living no longer opened with snapshot bytes). Root fix: PATH snapshot regenerated through R1-023; rule going forward — entries append BELOW the end-marker (this entry is the first under the rule; marker-literal compliance).
**Verifier.** `contract_closure.py`: `no-pending-phase1-fields` (post-freeze zero-tolerance over prereg/*.yaml+*.md) + `legitimate-pending-preserved` (`PENDING-owner-phase-execution` + `PENDING-HUMAN` still present) + `bound-count-asserted` == 43. Full run CLOSED 34/34 post-repair.
**Reruns.** contract_closure CLOSED; pytest 57/57 (10+12+9+26); manifest recompute == recorded; Path prefix holds; proof_status snapshot equality holds; ratified bytes `git diff --quiet` clean; claim dirs result-empty; runners 21/21 exit 2.
**Pending inventory.** `PENDING-Phase-1` in prereg: 0 (grepped). Legitimate states intact: corpus `PENDING-owner-phase-execution`, dual policy `PENDING-HUMAN`, theorem packages `PENDING-HUMAN`, proofs `PENDING owner phase`.
**Deviations.** None. F1 variant label `SOURCE_AVAILABLE-28` retained verbatim (frozen in PHASE02 output; membership governed by G1+H1, not the label).
**Verdict.** PENDING_PHASE1_PLACEHOLDERS = 0; WP1_ALL_GATES = PASS; recertification holds. WP-1 = COMPLETE.
